...
func verificationUpdate(table *Table) {
	...
	for name,col := range list1 {

		ishave := false
		for name2,col1 := range table2.columns {

			if col.EqualByComment(col1) {
				updateList[name] = col
				updateList[name].oldName = name2
				delete(table2.columns,name2)
				ishave = true
			}
		}
		if !ishave {
